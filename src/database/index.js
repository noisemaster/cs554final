...
this.insertUserIntoTable = async function (dbPool, tableName, user) {
    if (!dbPool) {
        throw 'Invalid dbPool Object';
    }
...
this.getUserByUsername = async function (dbPool, tableName, username) {
    ...
    try {
        ...
        result = await client.query(format(selectQuery, tableName, usersname));
    } catch (ex) {
        console.error(ex);
    } finally {
        client.release();
...
this.getAllEmails = async function (dbPool, tableName) {
    if (!dbPool) {
    ...
    const queryText = 'SELECT id, email FROM %I WHERE email<>"";';
